...
// SVN $Revision: 49537 $
// SVN $Date: 2025-08-15 00:27:52 +0530 (Fri, 15 Aug 2025) $
//
//
// *********************************************************************/
`timescale 1ns / 1ns
...
module caxi4interconnect_DWC_UpConv_Wchan_WriteDataFifoCtrl #
	...
	(
        ...
        input wire [11:0]  initiator_beat_cnt_shifted,
        input wire [11:0]  initiator_beat_cnt_to_boundary_shifted,
		...
		input wire        initiator_beat_cnt_eq_0,
  ...
  // assign mask_addr_msb = ~(size_one_hot - 1);
	...
	assign offset        = valid_fixed_flag ? 6'b0 : (second_wrap_burst ? initiator_beat_cnt_to_boundary_shifted[5:0] : ((pass_data) ? initiator_beat_cnt_shifted[5:0] : beat_cnt_reg_shifted[5:0]));
	...
	// if WRAP burst needs to be split into two transactions, increments the pkt counter when the caxi4interconnect_UpConverter receives enough data to provide a full transacton to the target otherwise wait for WLAST.
	assign boundary_reached  = (beat_cnt == {3'b000, to_boundary});
	// assign aligned_wrap      = (to_boundary == wlen_intr);
	...
	always @(posedge ACLK or negedge arst_sync) begin
		if (~arst_sync | ~srst_sync) begin
			...
			if (pass_data) begin
				...
				beat_cnt_reg_shifted <= {4'b0000, beat_cnt} << size;
			end
		end
	end
			...
			always @(posedge ACLK or negedge arst_sync) begin
				if (~arst_sync | ~srst_sync) begin
					...
					if (data_fifo_nearly_full) begin
						...
						else if ((valid_fixed_flag | (initiator_beat_cnt_eq_0)) & (j < addr_offset[WIDTH_OUT_ENCODED-1:WIDTH_IN_ENCODED]) & en_write) begin // zero data and strobes if the incoming address is not aligned to the output datawidth
							zero_strb_data_reg[j] <= 1'b1;
						end
	...
	generate
		for (j=0;j<WIDTH_IN_BYTES;j=j+1) begin
			always @(*) begin
				...
				else if ((valid_fixed_flag | (initiator_beat_cnt_eq_0)) & (j < addr_offset[WIDTH_IN_ENCODED-1:0]) & accept_data) begin // zero data in case of unaligned address at the beginning of a transaction
					zero_hold_reg_data[j] = 1'b1;
				end
