...
module caxi4pc_corefifo_sync_scntr (
    clk,
    //reset,
	aresetn,
	sresetn,
   ...
   //---------
    ...
    input  [WRITE_DEPTH-1:0] afull_in;             // almost full dynamic value
...
////////////////////////////////////////////
   ...
   // --------------------------------------------------------------------------
    ...
    begin
       if (!aresetn | !sresetn) begin
	    ...
	    if(load_prevpkt_waddr)
		    sc_r <= waddr_hold;
		else if( we_f_i ^ re_i) begin
			sc_r <= sc_r_cmb;
		end
	   end
       else begin
   ...
   generate
      ...
      // threshold values
      // --------------------------------------------------------------------------
      assign afthreshi =  AF_FLAG_STATIC ? AFULL_VAL    : AF_FLAG_DYN ? afull_in : FULL_WRITE_DEPTH;
      assign aethreshi =  AE_FLAG_STATIC ? AEMPTY_VAL   : 2;    //SAR#60185
     ...
     begin
        if ( !aresetn | !sresetn ) begin
           memwaddr_r <= 'h0;
			...
			if(load_prevpkt_waddr==1) 
			     memwaddr_r <= waddr_hold; 
            else if ( we_ctrl == 1'b1) begin																			// if(load_prevpkt_waddr==1)
              if(memwaddr_r == (FULL_WRITE_DEPTH-1)) begin    //SAR#68070                                   //	 memwaddr_r <= waddr_reg;
                 memwaddr_r <= 'h0;
				end 
			  else begin
