...
uint32_t I2C_BASE_ADDR = 0x41000000;
uint8_t I2C_DEV_ADDR = 0x4E;

// Fixed Offsets (Standard AXI IIC Map)
uint32_t I2C_CTRL_OFFSET  = 0x00;
uint32_t I2C_STAT_OFFSET  = 0x04;
uint32_t I2C_DATA_OFFSET  = 0x08;
uint32_t I2C_ADDR0_OFFSET = 0x0C;

// Control Register Bit Masks (CTRL)
uint8_t CTRL_CR2   = (1 << 7); // Clock Rate 2
uint8_t CTRL_ENS1  = (1 << 6); // Enable I2C Serial System
uint8_t CTRL_STA   = (1 << 5); // Start Flag
uint8_t CTRL_STO   = (1 << 4); // Stop Flag
uint8_t CTRL_SI    = (1 << 3); // Serial Interrupt (Clear to proceed)
uint8_t CTRL_AA    = (1 << 2); // Assert Acknowledge
uint8_t CTRL_CR1   = (1 << 1); // Clock Rate 1
uint8_t CTRL_CR0   = (1 << 0); // Clock Rate 0


#define CTRL_BASE (CTRL_ENS1 | CTRL_CR2)


// Common Status Codes (STAT)
uint8_t STAT_START_SENT     = 0x08;
uint8_t STAT_REP_START_SENT = 0x10;
uint8_t STAT_SLAW_ACK       = 0x18; // Slave Address Write Acked
uint8_t STAT_SLAW_NACK      = 0x20; // Slave Address Write NAcked
uint8_t STAT_DATAW_ACK      = 0x28; // Data Write Acked
uint8_t STAT_SLAR_ACK       = 0x40; // Slave Address Read Acked
uint8_t STAT_DATAR_ACK      = 0x50; // Data Read Acked
uint8_t STAT_DATAR_NACK     = 0x58; // Data Read NACK (End of read)
uint8_t STAT_IDLE           = 0xF8;

uint32_t GPIO_BASE_ADDR = 0x40000000;
uint32_t GPO_CFG_OFFSET = 0x00;
uint32_t EN_CFG_OFFSET = 0x04;
uint32_t OUT_OFFSET = 0xA0;
uint8_t GPO_MASK = (1 << 0);
uint8_t EN_MASK = (1 << 1);
...
uint32_t TX_BUF_ADDR = 0xC0000000UL;
uint32_t RX_BUF_ADDR = 0xC0100000UL;

uint32_t PROTOCONV_BASE_ADDR = 0xE0000000UL;

// --- S2MM (RX - Stream to Memory Mapped) Offsets ---
uint32_t PROTOCONV_S2MM_CTRL_OFFSET = 0x0010;
uint32_t PROTOCONV_S2MM_STS_OFFSET =  0x0014;
 uint32_t PROTOCONV_S2MM_LEN_OFFSET =  0x0018;
uint32_t PROTOCONV_S2MM_ADDR0_OFFSET = 0x001C;
uint32_t PROTOCONV_S2MM_ADDR1_OFFSET = 0x0020;

// --- MM2S (TX - Memory Mapped to Stream) Offsets ---
uint32_t PROTOCONV_MM2S_CTRL_OFFSET = 0x0410;
uint32_t PROTOCONV_MM2S_STS_OFFSET =  0x0414;
uint32_t PROTOCONV_MM2S_LEN_OFFSET =  0x0418;
uint32_t PROTOCONV_MM2S_ADDR0_OFFSET = 0x041C;
uint32_t PROTOCONV_MM2S_ADDR1_OFFSET = 0x0420;

uint32_t PROTOCONV_START_BIT = 0x00000001;
uint32_t PROTOCONV_INCR_BURST = 0x00000002;
uint32_t PROTOCONV_DONE_BIT = 0x00000001;

uint32_t FIR_HLS_BASE_ADDR = 0xE0010000UL;

//constexpr uint32_t BLOCK_SIZE = 256;
//constexpr uint32_t DATA_LENGTH = 4; //bytes

#define smpl_rd_num 3
#define coeff_size ((1 << (smpl_rd_num - 1)) * 128)

static PCDMA_instance_t g_pcdma_inst;
static unsigned char* g_dma_buff_base = NULL;
static unsigned long g_dma_buff_size = 0;
static unsigned long g_transfer_size = 1024;

volatile unsigned char* dma_active_buffer_half = NULL;
volatile unsigned char* eth_active_buffer_half = NULL;
volatile bool new_dma_data_ready = false;
...
{
    uint32_t int_src_register = PCDMA_S2MM_get_int_src(&g_pcdma_inst);
    uint32_t error_mask = S2MM_IE_AXI4_ERR_IRQ | S2MM_IE_PKT_DROP_OVF_IRQ | S2MM_IE_PKT_DROP_ERR_IRQ;

    if ((int_src_register & S2MM_IE_DONE_IRQ) && !(int_src_register & error_mask))
    {
        PCDMA_S2MM_clr_int_src(&g_pcdma_inst, S2MM_IE_DONE_IRQ);

        if (dma_active_buffer_half == g_dma_buff_base) {
            eth_active_buffer_half = g_dma_buff_base;
            dma_active_buffer_half = g_dma_buff_base + g_transfer_size;
        } else {
            eth_active_buffer_half = g_dma_buff_base + g_transfer_size;
            dma_active_buffer_half = g_dma_buff_base;
        }

        PCDMA_S2MM_configure(
                &g_pcdma_inst,
                (uint32_t)(g_transfer_size),
                (uint64_t)(dma_active_buffer_half),
                0x00u,
                PCDMA_BURST_TYPE_INCR
            );
        PCDMA_S2MM_start_transfer(&g_pcdma_inst);
        new_dma_data_ready = true;
    }
    else if (int_src_register & error_mask)
    {
        PCDMA_S2MM_clr_int_src(&g_pcdma_inst, int_src_register & error_mask);
    }
...
int dma_it_init(unsigned char *buff, unsigned long buff_size, unsigned long transfer_size){
    g_dma_buff_base = buff;
    g_dma_buff_size = buff_size;
    ...
    PCDMA_init(&g_pcdma_inst, PROTOCONV_BASE_ADDR);
    PCDMA_S2MM_disable_irq(&g_pcdma_inst, S2MM_IE_DONE_IRQ | S2MM_IE_AXI4_ERR_IRQ);

    PLIC_init();
    PLIC_SetPriority(PLIC_F2M_1_INT_OFFSET, 2);
    PLIC_EnableIRQ(PLIC_F2M_1_INT_OFFSET);
    __enable_irq();

    PCDMA_S2MM_enable_irq(&g_pcdma_inst, S2MM_IE_DONE_IRQ);
    PCDMA_S2MM_configure(
            &g_pcdma_inst,
            (uint32_t)(g_transfer_size),
            (uint64_t)(dma_active_buffer_half),
